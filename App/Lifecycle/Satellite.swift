...
final class Satellite {
    ...
    private(set) var gain: Percentage = AppSettings.shared.audioGain
    private(set) var vocalBoost: Percentage = AppSettings.shared.audioVocalBoost
...
private extension Satellite {
    ...
    func setupObservers() {
        ...
        AudioPlayer.shared.events.gainChanged
            ...
            .store(in: &observerSubscriptions)
        AudioPlayer.shared.events.vocalBoostChanged
            .sink { [weak self] boost in
                Task { @MainActor [weak self] in
